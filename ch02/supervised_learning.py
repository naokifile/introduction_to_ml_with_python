...
linear_smv = LinearSVC().fit(X, y)
print("Coefficient shape: ", linear_smv.coef_.shape)
print("Intercept shape: ", linear_smv.intercept_.shape)


mglearn.discrete_scatter(X[:, 0], X[:, 1], y)
line = np.linspace(-15, 15)
for coef, intercept, color in zip(linear_smv.coef_, linear_smv.intercept_,
                                  ["b", "r", "g"]):
...
mglearn.plots.plot_2d_classification(linear_smv, X, fill=True, alpha=.7)
mglearn.discrete_scatter(X[:, 0], X[:, 1], y)
line = np.linspace(-15, 15)
for coef, intercept, color in zip(linear_smv.coef_, linear_smv.intercept_,
                                  ["b", "r", "g"]):
